...
public class SqlConstants {
	...
	public static final String _LOGIN_CHECK = "select logintype from userdata where username=? and password=?";

	public static final String _REGISTER_USER = "INSERT INTO userdata VALUES((select nvl(max(userid),10)+1 from userdata),?,?,?,?,?,?)";

	public static final String _REGISTER_USER1 = "INSERT INTO userdata VALUES((select nvl(max(userid),1000)+1 from userdata),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public static final String _CHANGE_PASSWORD = "update userdetails set password=? where username=? and password=? ";
	...
	public static final String _VIEW_USERS = "SELECT USERID,FULLNAME,USERNAME,PHONENUMBER,EMAIL FROM USERDETAILS WHERE LOGINTYPE='user'";
	...
	// public static final String
	// _VIEW_FORWARDEDFILES_LIST="SELECT DISTINCT FILENAME,SENDFROM FROM FORWARDDATA WHERE SENDTO=(select userid from userdetails where username=upper(?))";
