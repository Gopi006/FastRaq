...
public class UserDAO extends AbstractDataAccessObject {
	...
	/*
	 * public boolean uploadData(String fileName, String userName) { boolean
	 * flag=false; Key pubK=null; Key priK=null; int no_of_servers=0; int
	 * block=0; String file=fileName.substring(fileName.lastIndexOf("\\")+1,
	 * fileName.length()); try{ con=new
	 * AbstractDataAccessObject().getConnection();
	 * System.out.println("Con..."+con
	 * +"..."+userName+"...........file........."+fileName+"''';''"+file);
	 * String sql=SqlConstants._NO_OF_SERVERS; String
	 * sql1=SqlConstants._USER_KEYS; String
	 * sql2=SqlConstants._ADD_CLOUDSERVER_DATA; ps=con.prepareStatement(sql);
	 * rs=ps.executeQuery(); if(rs.next()) { no_of_servers=rs.getInt(1); }
	 * ps1=con.prepareStatement(sql1); ps1.setString(1,userName);
	 * rs1=ps1.executeQuery();
	 */
	/*
	 * if(rs1.next()) {
	 * 
	 * byte[] bArray=null,bArray1=null; System.out.println("3");
	 * bArray=rs1.getBytes(2); bArray1=rs1.getBytes(3); InputStream input = new
	 * ByteArrayInputStream(bArray); ObjectInputStream ois=new
	 * ObjectInputStream(input); pubK=(Key) ois.readObject();
	 * System.out.println(pubK); InputStream input1 = new
	 * ByteArrayInputStream(bArray1); ObjectInputStream ois1=new
	 * ObjectInputStream(input1); priK=(Key) ois1.readObject();
	 * //System.out.println(priK);
	 * 
	 * 
	 * 
	 ...
	 * BufferedReader(fr); String strLine=""; String fileData="";
	 * System.out.println("........"); System.out.println("........"); while
	 * ((strLine = br.readLine()) != null) {
	 * 
	 * fileData = fileData + strLine;
	 * 
	 * } byte[] filesize=fileData.getBytes(); //RSA rsa=new RSA(); byte[] b; int
	 * loops,si,sj; //String st=""; int size=filesize.length; // Encrypt data
	 * into number of blocks for(loops=0,si=0;size>0;loops++) {
	 * System.out.println("blocks...."+loops); // String st="st"+loops; //String
	 * st=""; // if(size>116) //sj=si+116; else //sj=si+size;
	 * //st=fileData.substring(si,sj); // b=rsa.RSA_Enc(st,pubK); String
	 * codeword=Base64.encode(b); int temp=no_of_servers; // Replicate data
	 * for(int i=0;temp>0;i++){ // PreparedStatement pstm=null;
	 * 
	 * pstm=con.prepareStatement(sql2); pstm.setInt(1,temp);
	 * pstm.setString(2,userName); pstm.setString(3,file); pstm.setInt(4,loops);
	 * pstm.setBytes(5,b); pstm.setString(6,codeword); int
	 * rows=pstm.executeUpdate(); if(rows>0) flag=true; else flag=false;
	 * 
	 * System.out.println("loops...."+loops+"...."+i);
	 * 
	 * 
	 * System.out.println("server...."+no_of_servers); temp=temp-1;
	 * System.out.println("serveraaa...."+no_of_servers); }
	 * 
	 * size=size-116; si=sj; } } ps=con.prepareStatement(sql);
	 * ps.setString(1,userName); int rows=ps.executeUpdate(); if(rows>0){
	 * flag=true; System.out.println("....."); }
	 * 
	 * }catch(NullPointerException ne){ ne.printStackTrace(); //throw new
	 * DataAcessException("Data base connection Failed"); }
	 */
	/*
	 * catch(Exception se){ se.printStackTrace(); //throw new
	 * DataNotFoundException("Sql exception riser/datanot found"); }finally{
	 * try{ ps.close(); con.close(); }catch(Exception e){ e.printStackTrace(); }
	 * }
	 * 
	 * return flag; }
	 * 
	 ...
	 * ("select distinct gname from usergroup  order by gname");
	 * 
	 * rs=pstm.executeQuery(); awf=new ArrayList<RegisterBean>();
	 * 
	 * while(rs.next()){
	 * 
	...
	public int uploadDataa(String fileName, String userName) {
		...
		try {
			...
			if (rs1.next()) {
				...
				blocks = blocks + 1;
				System.out
						.println(""
								+ blocks);
			}
	...
	public ArrayList<DataBean> viewData(String userid, String fileName)
		...
		try {
			...
			while (rs2.next()) {
				...
				/*
				 * RSA rsa=new RSA(); String s=rsa.RSA_Dnc(decodedData,priK);
				 * System.out.println("decp...."+s+"...."+rs2.getInt(4));
				 * sb.setDec_msg(s); sb.setStorageid(no_of_servers);
				 * sb.setUserName(userid); sb.setFileName(fileName); al.add(sb);
				 */
			}
	...
	public ArrayList<DataBean> viewForwardData(int userid, String fileName)
		...
		try {
			...
			while (rs2.next()) {
				...
				System.out.println("bArray...." + bArray);
				/*
				 * RSA rsa=new RSA(); String s=rsa.RSA_Dnc(ChiperMsg,priK);
				 * System.out.println("decp...."+s+"...."+rs2.getInt(4));
				 * sb.setDec_msg(s); sb.setStorageid(no_of_servers); //
				 * sb.setUserName(userid); sb.setFileName(fileName);
				 */
				al.add(sb);
			}
